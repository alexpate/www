...
To better understand the problem, let's look at an example where we have a series of promises, each relying on the result of the previous one:
...
Recently, I discovered the [safe-await](https://github.com/DavidWells/safe-await) package, which provides a similar pattern for JavaScript promises. Here's how we can rewrite our example using safe-await:
